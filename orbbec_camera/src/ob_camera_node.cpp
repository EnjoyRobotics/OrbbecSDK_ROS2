...
OBCameraNode::OBCameraNode(
  rclcpp::Node * node, std::shared_ptr<ob::Device> device,
  ...
  try {
    ...
    for (size_t i = 0; i < filter_list->count(); i++) {
      ...
      if (filter_name == "DecimationFilter" && enable_decimation_filter_) {
        ...
        {
          RCLCPP_ERROR_STREAM(
            logger_, "Decimation filter scale value is out of range "
              << range.min << " - " << range.max);
        }
        ...
        if (threshold_filter_min_ != -1 && threshold_filter_max_ != -1) {
          RCLCPP_INFO_STREAM(
            logger_, "Set threshold filter value range to "
        ...
        if (temporal_filter_diff_threshold_ != -1.0 && temporal_filter_weight_ != -1.0) {
          RCLCPP_INFO_STREAM(
            logger_, "Set temporal filter value to "
        ...
        OB_PERMISSION_READ_WRITE))
    {
      ...
      std::transform(
        sync_mode_str_.begin(), sync_mode_str_.end(), sync_mode_str_.begin(),
        ::toupper);
    ...
    } else if (device_->isPropertySupported(
...
void OBCameraNode::setupProfiles()
{
    ...
    if (enable_stream_[elem]) {
      ...
      try {
          ...
          format_[elem] == OB_FORMAT_UNKNOWN)
        {
          ...
          selected_profile = profiles->getVideoStreamProfile(
            width_[elem], height_[elem],
  ...
  for (const auto & stream_index : HID_STREAMS) {
    ...
    try {
      ...
      RCLCPP_INFO_STREAM(
        logger_, "Failed to setup << " << stream_name_[stream_index]
...
void OBCameraNode::startStreams()
{
  ...
  try {
    setupPipelineConfig();
    pipeline_->start(
      pipeline_config_, [this](const std::shared_ptr<ob::FrameSet> & frame_set) {
...
void OBCameraNode::startIMUSyncStream()
{
  ...
  imuPipeline_->start(
    imuConfig, [&](std::shared_ptr<ob::Frame> frame) {
...
void OBCameraNode::getParameters()
{
  ...
  setAndGetNodeParameter<float>(
    temporal_filter_diff_threshold_, "temporal_filter_diff_threshold",
    -1.0);
  ...
  setAndGetNodeParameter<bool>(
    retry_on_usb3_detection_failure_, "retry_on_usb3_detection_failure",
    false);
}
...
void OBCameraNode::setupPublishers()
{
  ...
  if (enable_colored_point_cloud_) {
    depth_registration_cloud_pub_ = node_->create_publisher<PointCloud2>(
      ...
      rclcpp::QoS(
        rclcpp::QoSInitialization::from_rmw(point_cloud_qos_profile),
        point_cloud_qos_profile));
  }
  if (enable_point_cloud_) {
    depth_cloud_pub_ = node_->create_publisher<PointCloud2>(
      "depth/points", rclcpp::QoS(
        rclcpp::QoSInitialization::from_rmw(point_cloud_qos_profile),
        point_cloud_qos_profile));
  }
  auto device_info = device_->getDeviceInfo();
  ...
  for (const auto & stream_index : IMAGE_STREAMS) {
    ...
    camera_info_publishers_[stream_index] = node_->create_publisher<CameraInfo>(
      topic, rclcpp::QoS(
        rclcpp::QoSInitialization::from_rmw(camera_info_qos_profile),
...
void OBCameraNode::publishColoredPointCloud(const std::shared_ptr<ob::FrameSet> & frame_set)
{
  ...
  if (depth_width != color_width || depth_height != color_height) {
    RCLCPP_DEBUG(
      logger_, "Depth (%d x %d) and color (%d x %d) frame size mismatch", depth_width,
...
void OBCameraNode::onNewIMUFrameSyncOutputCallback(
  const std::shared_ptr<ob::Frame> & accelframe,
...
void OBCameraNode::publishStaticTF(
  const rclcpp::Time & t, const tf2::Vector3 & trans,
...
void OBCameraNode::calcAndPublishStaticTransform()
{
  ...
  for (const auto & item : stream_profile_) {
    ...
    try {
      ...
      RCLCPP_ERROR_STREAM(
        logger_, "Failed to get " << stream_name_[stream_index]
    ...
    RCLCPP_INFO_STREAM(
      logger_, "Publishing static transform from " << camera_link_frame_id_
                                                   << " to "
                                                   << stream_name_[stream_index]);
    RCLCPP_INFO_STREAM(logger_, "Translation " << trans[0] << ", " << trans[1] << ", " << trans[2]);
    RCLCPP_INFO_STREAM(
      logger_, "Rotation " << Q.getX() << ", " << Q.getY() << ", " << Q.getZ()
                           << ", " << Q.getW());
...
void OBCameraNode::publishDynamicTransforms()
{
  ...
  while (rclcpp::ok() && is_running_) {
    tf_cv_.wait_for(
      lock, std::chrono::milliseconds((int)(1000.0 / tf_publish_rate_)),
